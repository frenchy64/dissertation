...
To handle this, we distinguish two kinds of top-level expressions and attach
markers on AST nodes to keep track of them.
...
which further marks a top-level expression for evaluation.
...
The entry point here is \clj{check-top-level}, which has been enhanced
...
Then, once \clj{analyze-outer} analyzes the outer expression as a \clj{do},
its children are then marked with \clj{mark-top-level}.
...
Then, at time 14 \clj{eval-top-level} evaluates the \clj{defmacro}
expression (after it has been checked).
This repeats similarly for the final expression \clj{(mac)}.

There are a few notable cases to note where \clj{eval-top-level}
does \emph{not} trigger evaluation.
For example, at time 24, the outer-most \clj{do} is marked by
\clj{mark-top-level} only. Evaluation is skipped here because
the top-level markers have been passed along to its children
(at times 4 and 5)
which have already been evaluated
(at times 14 and 21), and so evaluating at time 24 would 
trigger an incorrect double-evaluation of the AST children.
Another case where evaluation is skipped is when an AST node has no top-level markings,
...
Incorporating correct top-level evaluation into a \texttt{core.typed.analyzer}
type checking loop is straightforward, and demonstrated in
...
must carefully manage markers to prevent double expansions.
...
are marked with \clj{mark-top-level} are evaluated.
...
(defn check-expr
  ...
  [expr expected]
  (if (= :unanalyzed (:op expr))
    ... (*@\label{analyzer:listing:cta-top-level-driver:unanalyzed-cases}@*)
