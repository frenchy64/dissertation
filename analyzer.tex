...
In this section, we describe with the subtleties of evaluating Clojure code.
...
Under evaluation, all forms are considered top-level 
until they are found to be nested under a non-\clj{do}
form---a top-level \clj{do} form grants top-level status
to each of its arguments.
That is, \clj{a} will be completely defined before
\clj{b} is expanded in \clj{(do (def a ...) (def b ...))}.
So, this arrangement means that the expansion of one top-level form
depends on the evaluation (and thus expansion) of all preceding top-level forms.
...
%- in contrast, Clojure always
%  evaluates code.
%- rely on -main etc.

%The fact that this later execution 


Usually both strategies evaluate identically---most differences come from
their handling of \emph{macroexpansion}.
