...
This seems to work in practice and is developed
by the same team that develops Clojure itself.

It is unclear whether the order-of-expansion of arguments to core macros in 
Clojure is intended to be stable across platforms.
For instance,
...
in ClojureScript.
While the order of evaluation of these macros must be preserved
...
The popular \texttt{tools.analyzer} library does not recognize
changes to the current namespace during macroexpansion. This is
...
in industry, it seems reasonable to conclude that expansion-time side effects
...
Relatedly,
in our experience from private conversations with Clojure developers,
programmers feel like avoiding repeated expansion
is important in Clojure, but cannot show an example of real-world code
that would fail with either semantics.
On the other hand, Racket developers were quick to demonstrate complex
...
Reflecting on these anecodes, Clojure developers are used to reloading
code ad-nausium and so are guided to writing code that can be re-evaluated
in almost any order (after an initial load), encouraged further by the late-binding semantics
of Clojure Vars.
On the other hand, Racket's module system is much more strict and reinitializes
...
It's interesting to note that the backgrounds and daily obligations of each groups varied wildly,
with the Racket programmers from academia (studying language extensibility)
and Clojure developers from industry.
Overall, while these differences in opinions relate to \emph{repeated} expansions,
it helps contextualize our larger discussion of
order-of-expansion for inner forms (which is unchanged
in the presence of code reloading,
since only top-level can be reevaluated in Clojure).
