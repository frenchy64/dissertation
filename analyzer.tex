...
on Java-style \clj{main} methods (invoked at the command line) to trigger initialization steps needed
in production.
...
starting from \clj{f}, whereas elements of unordered set literals \clj{#\{k*\}}
have an unspecified evaluation order.
On the other hand, order of expansion is not addressed.
Common Lisp asserts~\cite{steele1990common} that the order
of macroexpansion for inner forms is unspecified. This is a reasonable
...
Give that information with Clojure's strong Common Lisp heritage,
we think it is reasonable to assume the order of expansion
(of inner forms) is also explicitly unspecified in Clojure.
