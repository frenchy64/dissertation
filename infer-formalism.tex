...
\begin{mathpar}
  \begin{array}{lllll}
    ...
    \begin{array}{lllll}
      \text{where}
        &\overrightarrow{(\atenv{i}, \s{i}) = \aliashmap{}(\atenv{i-1},  \t{i})}\\
    \end{array}
    \\
    \aliashmap{}(\atenv{0}, \HMaptwo{\HMapreq{1}}{\HMapreq{2}}) = (\replacefor{\atenv{2}}{\HMaptwo{\HMapreqp{1}}{\HMapreqp{2}}}{\alias{}}, \alias{})\\
    \begin{array}{lllll}
      \text{where}
        ...
        \end{array}\\
        &\overrightarrow{(\atenv{i}, \overrightarrow{\t{i}}) = \textsf{foldr}(\textsf{step}, (\atenv{i-1}, []), rng(\HMapreq{i}))}\\
        &\overrightarrow{\HMapreqp{i} = \textsf{zip}(dom(\HMapreq{i}), \overrightarrow{\t{i}})}\\
        &\alias{} = freshAlias(\atenv{2})
    \end{array}
    \\
    \aliashmap{}(\atenv{0}, \Unionsplice{\t{1}, ..., \t{n}}) = \textsf{res}
    \\
    \begin{array}{lllll}
      \text{where}
      &\overrightarrow{(\atenv{i}, \s{i}) = \aliashmap{}(\atenv{i-1}, \t{i})}\\
      &\alias{} = freshAlias(\atenv{n})\\
      &\textsf{res} = \textsf{if} \bigwedge\overrightarrow{\textsf{simple?}(\s{i})}
                      \textsf{ then } (\atenv{n}, \Unionsplice{\s{1}, ..., \s{n}})
                      \textsf{ else } (\replacefor{\atenv{n}}{\Unionsplice{\overrightarrow{\textsf{resolveTopLevel}(\s{i})}}}{\alias{}}, \alias{})
    \end{array}
    \\
