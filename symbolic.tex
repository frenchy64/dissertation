...
% TODO is there an interesting example that uses nested (unannotated) functions
% such that giving the outer function an intersection type makes the 
% inner function hard to annotate?
% something like:
% (ann (fn [f] ((ann f [Int -> Int]) 1)) (IFn [[Int -> Int] -> Int] [[Num -> Num] -> Num]))
...
%output
\begin{cljlisting}
(let [f (fn [g :- (IFn [Int -> Int]
                       [Str -> Str])]
          (g 1)
          (g "a"))
      id (ann (fn [x] x) (IFn [Int -> Int]
                              [Str -> Str]))
      id2 (ann (fn [y] y) (IFn [Int -> Int]
                               [Str -> Str]))]
  (f id)
  (f id2))
\end{cljlisting}
...
\begin{figure}
  \begin{mathpar}
    ...
    \ltitjudgement{\ltiEnvp{}}
                  {\ltireannotatelhs{\ltiEnv{}}{\ltiT{}}{\ltiE{}}}
