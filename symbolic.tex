...
to the program size.
First, to demonstrate how the benchmark works, we choose a slightly smaller program size
($n=2$, where $n$ is the number of times size of the type duplicates).
...
The value \sml{fn z=> z}, assigned the type \sml{(?.X1 -> ?.X1)} (which we abbreviate as \sml{F}),
is duplicated in a pair, which is of type \sml{((F -> F -> ?.X2) -> ?.X2)} (which we abbreviate as \sml{G}),
which is then duplicated again, giving:
\sml{(G -> G -> ?.X3) -> ?.X3}.
