...
Unsurprisingly, this type checking with a naive implementation of symbolic
closures \emph{undecidable}.
...
For cases with more heterogenous argument and return types---like the y-combinator---the 
...
\caption{Benchmark demonstrating exponential growth of principal type schemes in ML.
  Types are manually abbreviated for readability.}
\label{symbolic:figure:pair-benchmark}
\end{figure}

The abbreviated output type \sml{it} is the result of the call to \sml{x3}
...
To start, \sml{x1}'s type is \sml{'a -> (('a -> 'a -> 'b) -> 'b)}---note that \sml{'a}
is duplicated, which provides a foundation for the exponential growth.
Then, \sml{x2}'s principal type is twice as large as \sml{x1}'s because it
iteratively calls \sml{x1} twice on its input---thus \sml{x2} duplicates its input \emph{four} times.
Extending this reasoning, \sml{x3}'s principal type is twice as large as \sml{x2}'s,
duplicating its input \emph{sixteen} times.
This number accounts for the number of times \sml{W} occurs in the output type.
