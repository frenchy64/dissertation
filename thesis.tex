...
For example, \clj{(tc ? GR)} asks to synthesize a type for the (provably untypable) \GRterm.
...
let $P$ stand for (copies of) the code \clj{(fn [x] (fn [z] (z x x)))} in the following, where the left
...
\clj{(fn [z] (z x x))} term occuring in $P$, whose definition type environment
...
grows exponentially.
We can reduce this to linear growth with sharing as below, where term returning \clj{xi} has type \clj{Pci}.
Each \clj{(fn [z] ...)} term comes from a different copy of $P$, in particular
the \clj{(fn [z] ...)} in type \clj{Pci} originates from the innermost $P$ in term \clj{xi}.
