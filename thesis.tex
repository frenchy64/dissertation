...
even in checking mode, because only the type of \ltiE{}
...
\]
%
uses symbolic execution to execute \text{id}, abstracting
results as types so they may be checked in typed regions.
Comparatively, symbolic closures deeply integrates a small amount of
symbolic execution with a type system in such a way that delayed symbolic computations
...
closures even use symbolic execution.
One view is that symbolic closures symbolically execute the \emph{runtime} closure-introduction rule.
%
\begin{mathpar}
\infer[]
  {}
...
%TODO
%Mix also uses symbolic execution for flow-sensitivity.
