...
could rewrite the check as \texttt{(assoc m :a ((fn [a] (inc a)) (get m :a)))},
...
functions, and we use an anonymous function, this is hard to type check.
However, combining custom type rules for \texttt{comp}, and \texttt{map}
with symbolic computations, we could simplify the original expression to
\texttt{(fn [a] (inc (dec a)))}, which is much easier to check.
...
a target language. Turnstile provides a fully extensible framework
for specifying building and combining core typing rules.
Typed Clojure does not have the goal of allowing users to override
...
Other work adds extensible systems
of \emph{type-specific languages}~\cite{omar2014safely}
to existing languages, which enables definitions of specialized syntax literals.
...
without requiring the user to write typing rules.
