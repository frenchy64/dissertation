...
Maps in Clojure also replace object-oriented programming in many situations.
...
with an explicit keyword \emph{dispatch entry} (e.g., \clj{:op}) to distinguish the
...
like \clj{Tree}, and annotating \clj{nodes} as
\begin{cljlisting}
(ann nodes [Tree -> Int])
\end{cljlisting}
...
as it is well maintained and generates similar or better annotations
as similar tools~\cite{saftoiu2010jstrace,pyannotate,typette18,An10dynamicinference,pytype,kristensen2017inference}.
...
\figref{fig:infer:typewiz} shows the actual output of TypeWiz for \figref{fig:infer:nodes}.
...
a common \js{nodes} method yields a similarly shallow annotation:
\begin{lstlisting}[language=JavaScript]
...
\end{lstlisting}
the annotation for \figref{fig:infer:nodes} is still not recursive (alarmingly, instead
growing linearly in number of nodes), and the class-based
example updates the type of \js{left} to \js{Leaf|Node},
but \js{right} is still \js{Leaf}.
%\begin{lstlisting}[language=JavaScript]
%class Node {               class Leaf { 
...
in the same file. Say \clj{nodes'}
is an alternative implementation of \clj{nodes},
...
(defalias @Op@
  (U (HMap
      :mandatory
      {@:op ':binding@,
       :info (U NameShadowMap FnScopeFnSelfNameNsMap),
       :local (U ':arg ':let ':fn),
       :name Sym}
      @:optional@
      {:arg-id Int,
    ...
    '{@:op ':fn-method@,
      ...
      :type nil,
      :variadic? false}
    ; omitted 10 cases w/ 16 references to Op
    ...))
\end{cljlisting}
\caption{Sample output of \texttt{core.typed.annotator}
         for the ClojureScript compiler's
         map-based AST format, named \clj{Op} here.
         It has 14 distinct operators, 5 inferred
         to have optional entries (like \clj{:op ':binding})
         and 22 inferred recursive references to \clj{Op}.
}
%    '{:op ':host-call,
%      :args '[Op],
%      :children Any,
